...
SAML.prototype.requestToUrl = function (request, operation, callback) {
    ...
    zlib.deflateRaw(request, function (err, buffer) {
        ...
        var target = new URL(self.options.entryPoint);

        if (operation === 'logout') {
            if (self.options.logoutUrl) {
                target = new URL(self.options.logoutUrl);
            }
        }
        ...
        for (var entry of Object.entries(samlRequest)) {
            target.searchParams.set(entry[0], entry[1]);
        }

        callback(null, target.toString());
    });
}
    ...
    parser.parseString(samlAssertion, function (err, doc) {
        ...
        if (assertion) {
            ...
            if (subject) {
                ...
                if (nameID) {
                    profile.nameID = nameID[0]["_"] ? nameID[0]["_"] : nameID[0];
